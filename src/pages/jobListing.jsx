...
const JobListing = () => {
  ...
  const [cities, setCities] = useState([]); // New state to store cities
  ...
  useEffect(() => {
    // Extract cities from the job data and remove duplicates
    if (jobs) {
      const availableCities = [...new Set(jobs.map((job) => job.location))];
